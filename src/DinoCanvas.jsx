...
function Dino({ divTop, divBottom }) {
    const { nodes, materials } = useGLTF('../public/models/dinoscene.glb');
    ...
    const handleScroll = () => {
        ...
        if (scrollBottom > dinoTopDistanceFromTop && scrollTop < dinoBottomDistanceFromTop) {
            moveJoint(nodes.neck002, (scrollTop - dinoTopDistanceFromTop) / window.innerHeight)
        }
    ...
    return (
        <mesh>
            <hemisphereLight skyColor={"black"} groundColor={0xffffff} intensity={5} position={[0, 5, 10]} />

            <pointLight color={"#ffaf8f"} intensity={150} position={[10, 5, 0]}></pointLight>
            <pointLight color={"#e6712e"} intensity={100} position={[10, 0, 15]}></pointLight>
            ...
            <group ref={dinoRef} position={[0, -2, 0]}>
                <group position={[-1, 0.408, 0]} rotation={[0.107, Math.PI / 2 + 0.2, 0]} scale={[1.15, 1.15, 1.15]}>
                    <skinnedMesh
